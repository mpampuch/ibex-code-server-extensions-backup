...
async function build(document) {
    const ast = await lw_1.lw.parse.tex(document.getText());
    if (!ast) {
 ...
 * iteration.
 */
function buildCommandPairTreeFromNode(doc, node, next, parentCommandPair, commandPairs) {
 ...
 * specified position.
 */
function walkThruForSurroundingPairs(pos, commandPairTree) {
 ...
 */
function walkThruForPairsNextToPosition(pos, commandPairTree) {
    const pairsAtPosition = [];
    if (commandPairTree.some((macroPair) => macroPair.startContains(pos) || macroPair.endContains(pos))) {
