...
function toPDF(args, forcedViewer = 'auto', pdfFile) {
    ...
    if (!pdfFile) {
        pdfFile = lw_1.lw.file.getPdfPath(rootFile);
    }
    ...
    if (useSyncTexJs) {
        try {
            ...
            const record = (0, worker_1.syncTeXToPDF)(line, filePath, pdfFile);
            if (!record) {
                return;
            }
            void lw_1.lw.viewer.locate(pdfFile, record);
        }
        catch (e) {
 ...
 */
function callSyncTeXToPDF(line, col, filePath, pdfFile) {
    const configuration = vscode.workspace.getConfiguration('latex-workshop');
 ...
 */
function callSyncTeXToTeX(page, x, y, pdfPath) {
    const configuration = vscode.workspace.getConfiguration('latex-workshop');
    const docker = configuration.get('docker.enabled');
