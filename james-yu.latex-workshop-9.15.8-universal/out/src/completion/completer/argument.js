...
class Argument {
    provideFrom(result, args) {
        ...
        const packages = lw_1.lw.completer.package.getPackagesIncluded(args.langId);
        let candidate;
        let environment;
        if (result[1] === 'begin') {
        ...
        for (const packageName of Object.keys(packages)) {
            if (environment) {
                const environments = lw_1.lw.completer.environment.getEnvFromPkg(packageName, environment_1.EnvSnippetType.AsCommand) || [];
                ...
                const commands = lw_1.lw.completer.command.getPackageCmds(packageName);
    ...
    providePackageOptions(line) {
        ...
        lw_1.lw.completer.loadPackageData(match[1]);
        const suggestions = lw_1.lw.completer.package.getPackageOptions(match[1])
            .map(option => {
            const item = new vscode.CompletionItem(option, vscode.CompletionItemKind.Constant);
            item.insertText = new vscode.SnippetString(option);
            return item;
    ...
    provideClassOptions(line) {
        ...
        lw_1.lw.completer.loadPackageData(isDefaultClass ? 'latex-document' : `class-${match[1]}`);
        const suggestions = lw_1.lw.completer.package.getPackageOptions(isDefaultClass ? 'latex-document' : `class-${match[1]}`)
            .map(option => {
            const item = new vscode.CompletionItem(option, vscode.CompletionItemKind.Constant);
            item.insertText = new vscode.SnippetString(option);
            return item;
