...
class Citation {
    constructor() {
        ...
        lw_1.lw.watcher.bib.onCreate(filePath => this.parseBibFile(filePath));
        lw_1.lw.watcher.bib.onChange(filePath => this.parseBibFile(filePath));
        lw_1.lw.watcher.bib.onDelete(filePath => this.removeEntriesInFile(filePath));
    }
    ...
    provide(uri, line, position) {
        ...
        return this.updateAll(this.getIncludedBibs(lw_1.lw.root.file.path)).map(item => {
    ...
    browser(args) {
        ...
        void vscode.window.showQuickPick(this.updateAll(this.getIncludedBibs(lw_1.lw.root.file.path)).map(item => {
            return {
    ...
    async parseBibFile(fileName) {
        ...
        const ast = await lw_1.lw.parse.bib(bibtex);
        if (ast === undefined) {
