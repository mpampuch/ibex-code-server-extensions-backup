...
const logger = lw_1.lw.log('Intelli', 'Command');
...
class Command {
    constructor() {
        ...
        const symbols = JSON.parse(fs.readFileSync(`${lw_1.lw.extensionRoot}/data/unimathsymbols.json`).toString());
        Object.entries(symbols).forEach(([key, symbol]) => this.defaultSymbols.push(this.entryCmdToCompletion(key, symbol)));
        lw_1.lw.onConfigChange(['intellisense.command.user', 'intellisense.package.exclude'], () => {
            this.initialize(lw_1.lw.completer.environment);
        });
    }
    initialize(environment) {
        ...
        const cmds = excludeDefault ? {} : JSON.parse(fs.readFileSync(`${lw_1.lw.extensionRoot}/data/commands.json`, { encoding: 'utf8' }));
        const maths = excludeDefault ? {} : JSON.parse(fs.readFileSync(`${lw_1.lw.extensionRoot}/data/packages/tex.json`, { encoding: 'utf8' })).cmds;
    ...
    provide(langId, line, position) {
        ...
        if ((configuration.get('intellisense.package.enabled'))) {
            ...
            Object.entries(packages).forEach(([packageName, options]) => {
                ...
                lw_1.lw.completer.environment.provideEnvsAsCommandInPkg(packageName, options, suggestions, defined);
            });
        }
        ...
        lw_1.lw.cache.getIncludedTeX().forEach(tex => {
            const cmds = lw_1.lw.cache.get(tex)?.elements.command;
            if (cmds !== undefined) {
                cmds.forEach(cmd => {
