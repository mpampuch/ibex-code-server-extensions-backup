...
function citation() {
    ...
    lw_1.lw.completer.citation.browser();
}
...
function devParseLog() {
    ...
    lw_1.lw.parse.log(vscode.window.activeTextEditor.document.getText());
}
...
async function devParseTeX() {
    ...
    const ast = await lw_1.lw.parse.tex(vscode.window.activeTextEditor.document.getText());
...
async function devParseBib() {
    ...
    const ast = await lw_1.lw.parse.bib(vscode.window.activeTextEditor.document.getText());
    return vscode.workspace.openTextDocument({ content: JSON.stringify(ast, null, 2), language: 'json' }).then(doc => vscode.window.showTextDocument(doc));
}
