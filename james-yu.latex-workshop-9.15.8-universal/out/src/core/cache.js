...
async function updateAST(fileCache) {
    ...
    fileCache.ast = await lw_1.lw.parse.tex(fileCache.content);
...
function updateElements(fileCache) {
    ...
    lw_1.lw.completer.citation.parse(fileCache);
    // Package parsing must be before command and environment.
    lw_1.lw.completer.package.parse(fileCache);
    lw_1.lw.completer.reference.parse(fileCache);
    lw_1.lw.completer.glossary.parse(fileCache);
    lw_1.lw.completer.environment.parse(fileCache);
    lw_1.lw.completer.command.parse(fileCache);
    lw_1.lw.completer.input.parseGraphicsPath(fileCache);
...
async function loadFlsFile(filePath) {
    ...
    for (const inputFile of ioFiles.input) {
        ...
        else if (!lw_1.lw.watcher.src.has(inputFile)) {
            // Watch non-tex files.
            add(inputFile);
        }
    }
