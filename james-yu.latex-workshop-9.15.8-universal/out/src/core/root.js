...
lw_1.lw.watcher.src.onDelete(filePath => {
    if (filePath !== exports.root.file.path) {
        return;
    }
...
function findFromMagic() {
    ...
    if (result) {
        ...
        content = lw_1.lw.file.read(filePath);
...
async function findInWorkspace() {
    ...
    try {
        ...
        for (const fileUri of fileUris) {
            ...
            const flsChildren = lw_1.lw.cache.getFlsChildren(fileUri.fsPath);
            ...
            if (result) {
                // Can be a root
                const children = lw_1.lw.cache.getIncludedTeX(fileUri.fsPath).filter(filePath => filePath !== fileUri.fsPath);
