...
exports.build = exports.autoBuild = void 0;
...
lw_1.lw.watcher.src.onChange(filePath => autoBuild(filePath, 'onFileChange'));
lw_1.lw.watcher.bib.onChange(filePath => autoBuild(filePath, 'onFileChange', true));
/**
...
function canAutoBuild() {
    ...
    return Date.now() - lw_1.lw.compile.lastBuildTime >= configuration.get('latex.autoBuild.interval', 1000);
}
/**
...
async function buildLoop() {
    if (lw_1.lw.compile.compiling) {
        return;
    }
    lw_1.lw.compile.compiling = true;
    lw_1.lw.compile.lastBuildTime = Date.now();
    ...
    while (true) {
        ...
        lw_1.lw.compile.compiledRootFile = step.rootFile;
...
function spawnProcess(step) {
    ...
    if (!step.isExternal &&
        ...
        if (args && !step.name.endsWith(lw_1.lw.constant.MAGIC_PROGRAM_ARGS_SUFFIX)) {
            ...
            lw_1.lw.compile.process = cs.spawn(`${step.command} ${args[0]}`, [], { cwd: path.dirname(step.rootFile), env, shell: true });
        }
        else {
            lw_1.lw.compile.process = cs.spawn(step.command, args, { cwd: path.dirname(step.rootFile), env });
        }
    }
        ...
        logger.log(`cwd: ${cwd}`);
        lw_1.lw.compile.process = cs.spawn(step.command, step.args, { cwd, env });
    }
    else {
        logger.log(`cwd: ${step.cwd}`);
        lw_1.lw.compile.process = cs.spawn(step.command, step.args, { cwd: step.cwd });
    }
...
async function monitorProcess(step, env) {
    ...
    const result = await new Promise(resolve => {
        ...
        lw_1.lw.compile.process.on('exit', (code, signal) => {
            const isSkipped = lw_1.lw.parse.log(stdout, step.rootFile);
            if (!step.isExternal) {
...
function handleExternalCommandError() {
    logger.log(`Build returns with error on PID ${lw_1.lw.compile.process?.pid}.`);
...
async function afterSuccessfulBuilt(lastStep, skipped) {
    ...
    lw_1.lw.viewer.refresh(lw_1.lw.file.getPdfPath(lastStep.rootFile));
    lw_1.lw.completer.reference.setNumbersFromAuxFile(lastStep.rootFile);
