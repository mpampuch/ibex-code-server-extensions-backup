...
class DefinitionProvider {
    ...
    provideDefinition(document, position) {
        ...
        if (token.startsWith('\\')) {
            const command = lw_1.lw.completer.command.definedCmds.get(token.slice(1));
            if (command) {
                return command.location;
            }
            return;
        }
        const ref = lw_1.lw.completer.reference.getRef(token);
        if (ref) {
            return new vscode.Location(vscode.Uri.file(ref.file), ref.position);
        }
        const cite = lw_1.lw.completer.citation.getEntry(token);
        if (cite) {
            return new vscode.Location(vscode.Uri.file(cite.file), cite.position);
        }
        const glossary = lw_1.lw.completer.glossary.getEntry(token);
        if (glossary) {
