...
lw_1.lw.parse = parse_1.parse;
void lw_1.lw.parse.reset();
...
const bibtex_1 = require("./completion/bibtex");
const symbol_document_1 = require("./language/symbol-document");
const symbol_project_1 = require("./language/symbol-project");
const definition_1 = require("./language/definition");
const folding_1 = require("./language/folding");
const selection_1 = require("./language/selection");
const latex_1 = require("./completion/latex");
const logger = lw_1.lw.log('Extension');
function initialize() {
    lw_1.lw.completer = new latex_1.Completer();
    lw_1.lw.atSuggestionCompleter = new latex_1.AtSuggestionCompleter();
...
function activate(extensionContext) {
    ...
    extensionContext.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(async (e) => {
        ...
        if (e && lw_1.lw.file.hasTexLangId(e.document.languageId) && e.document.fileName !== prevTeXDocumentPath) {
...
function registerProviders(extensionContext) {
    ...
    extensionContext.subscriptions.push(vscode.window.registerWebviewPanelSerializer('latex-workshop-pdf', lw_1.lw.viewer.serializer), vscode.window.registerCustomEditorProvider('latex-workshop-pdf-hook', lw_1.lw.viewer.hook, { supportsMultipleEditorsPerDocument: true, webviewOptions: { retainContextWhenHidden: true } }), vscode.window.registerWebviewPanelSerializer('latex-workshop-mathpreview', lw_1.lw.extra.mathpreview.serializer));
    extensionContext.subscriptions.push(vscode.languages.registerHoverProvider(latexSelector, lw_1.lw.preview.provider), vscode.languages.registerDefinitionProvider(latexSelector, new definition_1.DefinitionProvider()), vscode.languages.registerDocumentSymbolProvider(latexSelector, new symbol_document_1.DocSymbolProvider()), vscode.languages.registerDocumentSymbolProvider(bibtexSelector, new symbol_document_1.DocSymbolProvider()), vscode.languages.registerDocumentSymbolProvider(selectDocumentsWithId(['doctex']), new symbol_document_1.DocSymbolProvider()), vscode.languages.registerWorkspaceSymbolProvider(new symbol_project_1.ProjectSymbolProvider()));
    extensionContext.subscriptions.push(vscode.languages.registerCompletionItemProvider({ scheme: 'file', language: 'tex' }, lw_1.lw.completer, '\\', '{'), vscode.languages.registerCompletionItemProvider(bibtexSelector, new bibtex_1.BibtexCompleter(), '@'));
    ...
    const registerTrigger = () => {
        ...
        triggerDisposable = vscode.languages.registerCompletionItemProvider(latexDoctexSelector, lw_1.lw.completer, ...latexTriggers);
    ...
    const registerAtSuggestion = () => {
        ...
        if (atSuggestionLatexTrigger !== '') {
            lw_1.lw.atSuggestionCompleter.updateTrigger();
            atSuggestionDisposable = vscode.languages.registerCompletionItemProvider(latexDoctexSelector, lw_1.lw.atSuggestionCompleter, atSuggestionLatexTrigger);
    ...
    extensionContext.subscriptions.push(vscode.languages.registerCodeActionsProvider(latexSelector, lw_1.lw.lint.latex.actionprovider), vscode.languages.registerFoldingRangeProvider(latexSelector, new folding_1.FoldingProvider()), vscode.languages.registerFoldingRangeProvider(weaveSelector, new folding_1.WeaveFoldingProvider()));
    const selectionLatex = configuration.get('selection.smart.latex.enabled', true);
    if (selectionLatex) {
        extensionContext.subscriptions.push(vscode.languages.registerSelectionRangeProvider({ language: 'latex' }, new selection_1.SelectionRangeProvider()));
    }
